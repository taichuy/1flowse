...
export function formatManualResumeResultMessage(runStatus?: string | null) {
  if (!runStatus) {
    return "已发起恢复尝试，请立即回看当前 run 时间线，确认是否真正离开 waiting 状态。";
  }

  if (runStatus === "waiting") {
    return "已发起恢复尝试，但 run 仍处于 waiting。请继续检查 callback ticket、审批进度或定时恢复是否仍在阻塞。";
  }

  if (runStatus === "running") {
    return "已发起恢复尝试，run 已重新进入 running。接下来重点确认节点是否继续推进，而不只是停留在恢复事件。";
  }

  if (runStatus === "succeeded") {
    return "已发起恢复尝试，run 已完成 succeeded。当前阻塞链路已经解除。";
  }

  if (runStatus === "failed") {
    return "已发起恢复尝试，但 run 已落到 failed。请结合 blocker timeline 与节点错误继续排障。";
  }

  return `已发起恢复尝试，当前 run 状态：${runStatus}。请继续回看时间线确认这次恢复是否真正推动了执行。`;
}
...
export function formatApprovalDecisionResultMessage(decision: "approved" | "rejected") {
  if (decision === "approved") {
    return "审批已通过，runtime 会沿原 waiting 链路继续尝试恢复；如果 run 仍停在 waiting，请继续检查 callback 或定时恢复。";
  }

  return "审批已拒绝，对应等待中的执行会保持 blocked / failed 语义；后续应转向人工处理或重新发起新的访问请求。";
}
...
export function formatNotificationRetryResultMessage(input: {
  ...
  if (input.status === "delivered") {
    return `通知已重新投递到 ${targetLabel}。这一步只表示审批请求已送达；run 仍需等待人工决策后才会继续。`;
  }

  if (input.status === "pending") {
    return `通知已按 ${targetLabel} 重新入队，等待 worker 投递。当前 run 仍会保持 waiting，直到审批完成或阻塞解除。`;
  }
  ...
  return "通知已触发重试，请继续回看最新通知状态与审批结果，确认阻塞是否真正解除。";
}
