...
summary: `style-boundary` 工具只负责检测样式边界是否被打坏、文件影响面是否扩散失控，以及 page/component/style 三层映射是否声明清楚；不能把它当成泛 UI 质量或信息架构评估器。
keywords:
  - style-boundary
  - frontend
  - qa
...
`style-boundary` 只用于样式边界和样式扩散门禁，不用于泛 UI 质量判断。

# 原因

用户明确要求这套工具只约束样式有没有越界、有没有扩散、有没有守住页面/组件/样式三层边界；审美、信息架构和交互质量应由其他 QA 或设计评审手段负责。
...
命中文案应优先使用“样式边界失败 / 样式扩散失败”，避免写成泛 QA 失败。
