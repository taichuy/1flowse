...
topic: frontend-development skill 对 UI 开发需求必须先输出需求整理与细化
summary: 用户要求 `frontend-development` skill 不仅在模糊需求场景下思考需求，而且在页面 / UI 开发需求中应把需求整理、需求细化和明确建议作为显式回复内容先发给用户，再默认继续实现。
keywords:
  - frontend
  - skill
  - ui requirement
...
# frontend-development skill 对 UI 开发需求必须先输出需求整理与细化
...
- `frontend-development` skill 命中页面 / UI 开发需求时，回复里必须先显式输出需求整理、需求细化和明确建议。
