...
summary: 自 `2026-04-19 14` 起，`docs/superpowers/plans/2026-04-19-plugin-trust-source-install.md` 成为“签名信任底座 + 镜像源 + 上传插件安装”专题的执行计划。计划固定先补 `plugin-framework` 统一 intake pipeline，再补宿主持久化与 official/mirror source contract，随后接浏览器上传和设置页 source/trust UI。为保持 spec 的三类来源模型，遗留 `POST /api/console/plugins/install` 兼容路径在本轮只作为内部手工导入入口保留，持久化口径统一归到 `source_kind=uploaded`，不再延续 `downloaded_or_uploaded`。
keywords:
  - official-plugin
  - signature-trust
  - mirror-registry
