...
summary: 当前浅色翡翠 mock 的导航与壳层样式调整实际维护在 `tmp/mock-ui`，用户基于截图指出的导航问题应先改该目录，再评估是否回流 `web/`。
keywords:
  - mock-ui
  - tmp/mock-ui
  - navigation
...
AI 根据用户提供的 mock 截图和 `../dify/web` 参考，实现浅色翡翠壳层导航栏的高度与布局修正，实际修改目录为 `tmp/mock-ui`。
...
需要先让截图对应的 mock 演示与用户预期对齐，确保导航问题在当前可见产物里被修正，而不是只在源目录里理论修正。
...
优先保证“用户正在看的 mock”被正确修复，避免因为 `web/` 与 `tmp/mock-ui` 暂时不同步而把改动落到错误目录。
