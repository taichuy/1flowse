...
summary: 用户确认 `tmp/mock-ui` 的主题和导航审美方向后，正式 `web` 已回流浅色翡翠主题 token、顶部水平导航和右侧纯文本账户入口；当前顶栏文案固定为“工作台 / 团队 / 前台”，账户入口与右边缘保持 `5%` 间距，账户区最终采用 horizontal `SubMenu`，用户名在固定宽度区块中居中显示，下拉项文字按行居中且字号放大到更匹配 item 高度，且没有把 `theme-preview` 这类沙盒展示页一起带回主应用。
keywords:
  - web
  - tmp/mock-ui
  - theme
...
用户明确要求把 `tmp/mock-ui` 里更符合审美的主题和导航栏迁移到正式 `web`，并进一步把导航文案收敛为“工作台 / 团队 / 前台”，同时把右侧账户入口从贴边状态后退到距右边缘 `5%` 的位置。账户区最终不采用独立 `Dropdown`，而是改为 horizontal `SubMenu`，用户名在更宽的固定区块中居中显示，下拉项文字也做逐项居中，并把字号放大到更匹配 item 高度。AI 已将这些壳层规则落实到正式前端，并通过本地浏览器截图验证。
