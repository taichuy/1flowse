...
summary: 用户确认 `tmp/mock-ui` 的主题和导航审美方向后，正式 `web` 已回流浅色翡翠主题 token、顶部水平导航和右侧纯文本账户入口，但没有把 `theme-preview` 这类沙盒展示页一起带回主应用。
keywords:
  - web
  - tmp/mock-ui
  - theme
...
用户明确要求把 `tmp/mock-ui` 里更符合审美的主题和导航栏迁移到正式 `web`。AI 已将正式前端回流为浅色翡翠壳层：统一 `Ant Design` 主题 token、浅底工作台背景、顶部水平导航和右侧纯文本账户入口。
...
回流范围只覆盖正式应用真正生效的壳层能力，不把 `theme-preview` 这类沙盒展示页一起带回主应用，避免把探索用页面误当成正式产品结构。
