...
- 用户已补充确认五项具体策略：
  ...
  - 不做 `json_schema -> outputs` 反推，仍按普通 `LLM` 文本输出处理
...
- `json_schema` 虽然提前进入一期，但仍应作为节点级独立对象处理；它用于约束返回格式，不改变当前 `LLM` 节点只暴露 `text` 输出的事实。
