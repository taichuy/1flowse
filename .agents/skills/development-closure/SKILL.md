...
description: 用于 7Flows 一轮实现、文档或 skill 调整接近完成时，统一收尾验证、文档同步、Git 提交和下一步规划，避免遗漏 team-conventions、ADR、索引与本地私有连续性留痕等开发闭环。
---
...
- 需要确认是否补 `docs/dev/team-conventions.md`、`docs/adr/`、共享索引或当前开发者自己的 `docs/.private/` 连续性文档
...
如果任务涉及产品边界、OpenClaw 对外切口、开源项目定位、插件兼容、安全、变量传递或 runtime 架构，再补读：
...
- 当前开发者自己的稳定偏好、自治开发偏好与汇报口径：如确有需要，更新 `docs/.private/user-preferences.md`
- 当前开发者自己的当前事实、结构热点、近期优先级与下一步规划：如确有需要，更新 `docs/.private/runtime-foundation.md`
...
- 如果当前开发者需要维持自己的连续性事实或下一步优先级，就同步 `docs/.private/runtime-foundation.md`。
- 更新 `docs/.private/runtime-foundation.md` 时，为目标记录和下一步规划补 `YYYY-MM-DD HH:mm` 时间戳。
- 如果当前开发者需要维持自己的稳定自治偏好或汇报格式，就同步 `docs/.private/user-preferences.md`。
...
- 不要把用户已有的无关脏改动一起带进提交
- 只提交本轮相关文件
- 默认做一次非交互式提交，并尝试把当前分支推送到远端；如果本轮只是中间探索态，或推送失败 / 不适合推送，要在最终汇报里明确说明原因
