...
### 6. 是否把加载态、等待态错误做成“大段说明卡片”

- `loading.tsx`、bootstrap loading、局部 pending / waiting state 默认应是居中 `Spin`、必要的 `Skeleton` 或轻遮罩，而不是正文主区里的大块解释卡片。
- 如果底层壳层、导航或上下文已经存在，优先只遮罩当前活跃 surface；不要再插入一张带标题、摘要、阶段说明的大段 loading 卡。
- 只有当用户必须在等待中理解 blocker、审批结果或下一步动作时，才允许出现更完整的说明 surface；否则应直接指出为交互噪音和信息层级错误。
