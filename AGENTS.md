...
- 必须把 `docs/dev/runtime-foundation.md` 维持为“当前事实索引”而不是无限追加的流水账；当内容过长（例如超过约 1500 行或已明显削弱指导作用）时，应结合当前代码现状压缩到约 1000 行以内，并保留当前“下一步规划”作为最高优先级指引；历史明细另行归档到 `expired` 文档。
