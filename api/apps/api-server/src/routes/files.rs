...
fn map_file_storage_error(error: storage_object::FileStorageError) -> ApiError {
    match error {
        ...
        storage_object::FileStorageError::InvalidConfig(_) => {
            control_plane::errors::ControlPlaneError::Conflict("file_storage_config_invalid")
                .into()
        }
...
pub async fn upload_file(
    ...
    .upload(control_plane::file_management::UploadFileCommand {
        ...
        file_table_id: parse_uuid(
            file_table_id.as_deref().ok_or_else(|| invalid_input("file_table_id"))?,
            "file_table_id",
        )?,
