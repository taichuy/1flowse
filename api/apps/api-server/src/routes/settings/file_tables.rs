...
async fn to_response<R>(repository: &R, record: domain::FileTableRecord) -> Result<FileTableResponse, ApiError>
where
...
pub fn router() -> Router<Arc<ApiState>> {
    Router::new()
        .route("/file-tables", get(list_file_tables).post(create_file_table))
        .route(
            "/file-tables/:id/binding",
