...
async fn file_management_routes_create_workspace_table_upload_and_read_by_storage_snapshot() {
    ...
    let (admin_cookie, admin_csrf) = login_and_capture_cookie(&app, "file-admin", "change-me").await;
    ...
    let backup_root = std::env::temp_dir().join(format!("file-management-routes-{}", Uuid::now_v7()));
    ...
    assert_eq!(
        to_bytes(content_response.into_body(), usize::MAX).await.unwrap(),
        &b"hello file-management"[..]
    );
    ...
    let bind_response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .header("x-csrf-token", &admin_csrf)
                .header("content-type", "application/json")
                .body(Body::from(json!({ "bound_storage_id": Uuid::now_v7() }).to_string()))
                .unwrap(),
        )
        .await
        .unwrap();
