...
fn actor_in_workspace(workspace_id: Uuid) -> ActorContext {
    ActorContext::scoped(
        ...
        workspace_id,
        "admin",
        ["state_data.create.all".to_string(), "state_data.view.all".to_string()],
    )
}
...
async fn upload_service_writes_object_and_runtime_record_with_storage_snapshot() {
    ...
    let model = model_definition(model_id, DataModelScopeKind::Workspace, workspace_id, "assets");
    let root = temp_root("file-upload-service");

    repository.insert_file_storage(domain::FileStorageRecord {
        id: storage_id,
    ...
    let service =
        FileUploadService::new(repository.clone(), registry.clone(), runtime_engine_for_model(&model));
