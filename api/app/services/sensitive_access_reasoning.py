...
_REASON_LABELS: dict[str, str] = {
    ...
    "allow_human_moderate_runtime_use": (
        "Human moderate-sensitivity runtime access allowed"
    ),
    ...
    "approval_required_moderate_sensitive_operation": (
        "Moderate-sensitivity operation requires approval"
    ),
    "deny_non_human_high_sensitive_mutation": (
        "Non-human high-sensitivity mutation denied"
    ),
    ...
    "approval_expired": "Approval expired before review",
    "access_denied": "Access denied",
}

_POLICY_SUMMARIES: dict[str, str] = {
    "allow_low_sensitivity": (
        "Default policy allows low-sensitivity resources without extra review."
    ),
    "allow_standard_low_risk": (
        "Default policy allows this low-risk access pattern without operator review."
    ),
    "allow_human_moderate_runtime_use": (
        "Human read/use/invoke access to moderate-sensitivity resources is allowed "
        "by default policy."
    ),
    "masked_moderate_runtime_use": (
        "Non-human read/use/invoke access to moderate-sensitivity resources is only "
        "allowed in masked form."
    ),
    "approval_required_non_human_mutation": (
        "Non-human export or write operations require operator approval before "
        "execution can continue."
    ),
    "approval_required_moderate_sensitive_operation": (
        "This moderate-sensitivity operation crosses the default review threshold "
        "and must be approved."
    ),
    "deny_non_human_high_sensitive_mutation": (
        "Default policy blocks non-human export or write operations against "
        "high-sensitivity resources."
    ),
    "approval_required_high_sensitive_access": (
        "High-sensitivity access must be reviewed by an operator before the "
        "workflow can resume."
    ),
    ...
    "rejected_after_review": (
        "An operator rejected the request, so the blocked workflow remains denied."
    ),
    "approval_expired": (
        "The approval ticket expired before an operator responded, so the blocked "
        "workflow resumes along the denied path."
    ),
    ...
    *,
    ...
    return SensitiveAccessReasoning(
        decision_label=_DECISION_LABELS.get(
            normalized_decision,
            normalized_decision or "Pending",
        ),
