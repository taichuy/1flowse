...
def _serialize_provider_item(record, *, provider_catalog: NativeModelProviderDefinition, credential) -> WorkspaceModelProviderConfigItem:
...
def _serialize_registry_response(db: Session, *, workspace_id: str, include_inactive: bool) -> WorkspaceModelProviderRegistryResponse:
...
@router.post("", response_model=WorkspaceModelProviderConfigItem, status_code=status.HTTP_201_CREATED)
def create_workspace_model_provider_config(
    ...
    db.commit()
    db.refresh(record)
    credential = registry_service.list_credentials_index(db, credential_ids=[record.credential_id])[record.credential_id]
