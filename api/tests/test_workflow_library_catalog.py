...
def test_builtin_agent_starter_preserves_catalog_defaults_and_canvas_positions() -> None:
    ...
    agent_node = next(node for node in agent_starter.definition["nodes"] if node["id"] == "llmAgentNode")

    assert agent_node["type"] == "llmAgentNode"
    assert agent_node["name"] == "llmAgentNode"
    ...
    output_node = next(node for node in agent_starter.definition["nodes"] if node["id"] == "endNode")
...
def test_builtin_sandbox_code_starter_preserves_defaults_and_connects_mainline() -> None:
    ...
    sandbox_node = next(node for node in sandbox_starter.definition["nodes"] if node["id"] == "sandboxCodeNode")
    output_node = next(node for node in sandbox_starter.definition["nodes"] if node["id"] == "endNode")

    assert sandbox_starter.business_track == "编排节点能力"
    assert sandbox_node["type"] == "sandboxCodeNode"
    assert sandbox_node["name"] == "sandboxCodeNode"
    ...
    assert sandbox_starter.definition["edges"] == [
        {
            "id": "edge_startNode_sandboxCodeNode",
            "sourceNodeId": "startNode",
            "targetNodeId": "sandboxCodeNode",
            "channel": "control",
        },
        {
            "id": "edge_sandboxCodeNode_endNode",
            "sourceNodeId": "sandboxCodeNode",
...
def test_build_node_catalog_marks_planned_nodes_without_confusing_palette_visibility() -> None:
    ...
    trigger_node = next(item for item in node_catalog if item.type == "startNode")
    sandbox_node = next(item for item in node_catalog if item.type == "sandboxCodeNode")
    loop_node = next(item for item in node_catalog if item.type == "loopNode")
