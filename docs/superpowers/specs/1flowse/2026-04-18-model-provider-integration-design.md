...
5. 运行插件级测试
6. 打包为 `.1flowsepkg`
7. 对产物做 `checksum / signature`
8. 上传到官方 artifact storage，并写入 registry 元数据
...
2. 定义插件安装任务、installed registry 与 assignment 最小闭环
3. 定义 provider runtime contract 与标准事件流
4. 先做 `plugin-runner` 的最小 provider `load / invoke / stream` 闭环
5. 初始化 `../1flowse-official-plugins` 并落 `openai_compatible` 参考 plugin 源码与打包流程
6. 主仓库实现 schema 解析、provider instance 实体、控制面 service 与 route
7. 落 `Settings / 模型供应商` 页面
8. 升级 `LlmModelField` 到真实 provider-aware 选择器
9. 在编译阶段接入 provider 校验
10. 先用 `openai_compatible` 打通真实链路
11. 其他 provider 后续通过同一 contract 独立接入，不要求宿主改边界
