...
2. 数据模型由 `system` 统一创建，再按通用 `scope` 授权使用的规则。
3. 单机开源版不暴露 workspace，但核心保留轻量 `scope_id`。
4. 未来 workspace 多租户由 HostExtension 作为 scope provider 扩展。
5. 外部数据源通过插件接入，但统一进入平台模型层、权限层和审计层。
6. 自动 CRUD API、API Key、编排节点和前端入口的边界。
7. V1 范围、非目标、验收标准和后续计划入口。
...
5. 权限、审计、session、scope 语义已经形成基本边界。
...
`scope` 是权限层级和数据隔离边界，不等于物理数据源、物理表或 workspace 产品对象。

V1 核心只内置稳定 scope 语义，不把 workspace 写成内核必备对象：

1. `owner`：只能访问自己创建或归属自己的记录。
2. `scope_all`：只能访问当前 scope 内全部记录。
3. `system_all`：能访问所有 scope 的完整数据。

单机开源版默认只有一个内置 `DEFAULT_SCOPE_ID`。此时产品上不需要暴露 workspace，但动态表和运行时上下文仍保留 `scope_id`，为未来多 scope 能力提供稳定扩展点。

未来 workspace 多租户能力由 HostExtension 提供。核心只识别：

1. `actor.current_scope_id`
2. `actor.scope_permissions`
3. `record.scope_id`
4. 数据模型 scope grant

核心不关心这个 scope 来自 workspace、project、organization 还是其他 provider。
...
1. system 管理员：配置数据源、创建数据模型、配置 scope grant、管理全局权限。
2. scope 管理员：使用被授权的数据模型，配置当前 scope 内可见范围和 API 使用方式。单机开源版中该角色退化为默认 scope 管理者。
...
5. 数据模型只能由 system 创建，再通过 scope grant 授权给当前 scope 使用。
...
6. `main_source` 中的数据模型 code 由 system 统一治理，避免多个 scope 创建同名物理表。

### 7.3 system 建模，scope 使用
...
这样可以避免多个 scope 创建相同表名导致物理冲突，也能让 system 管理员统一治理数据模型生命周期。单机开源版只有 `DEFAULT_SCOPE_ID`，不会在产品层暴露 workspace，也不会引入 workspace 级管理负担。

### 7.4 Scope Provider 扩展

核心不内置 workspace 多租户产品形态。未来 workspace 能力由 HostExtension 作为 scope provider 扩展：
...
HostExtension 不直接改写动态表或运行时 CRUD 查询规则。核心查询层固定按 `scope_id` 和权限范围执行。
...
6. `scope_kind / scope_id`：实例归属。单机开源版使用 `DEFAULT_SCOPE_ID`；未来 workspace HostExtension 可提供多个 scope。
...
### 8.4 ScopeDataModelGrant

需要新增或等价表达“system 数据模型授权给某个 scope”的授权关系。
...
它不是复制数据模型，也不是 fork。它只是 scope 使用 system 数据模型的授权和配置入口。
...
单机开源版中所有记录默认写入 `DEFAULT_SCOPE_ID`。未来多 scope 模式中，写入时使用 `actor.current_scope_id`。
...
单机开源版固定 `actor.current_scope_id = DEFAULT_SCOPE_ID`。因此 `scope_all` 等价于当前默认环境内全部数据，不需要暴露 workspace UI，也不会引入工作空间切换查询成本。
...
不支持 owner/scope 过滤的外部数据源，不允许授予 `owner` 或 `scope_all` 范围；除非管理员显式用 `system_all` 风险授权，否则不得发布为可写运行时数据模型。
...
### V1.5 workspace HostExtension 预留验证
...
9. 外部数据源发布必须通过连接测试和 schema 校验。
10. 外部数据源 CRUD 通过插件执行，但权限和审计仍由平台控制。
11. 编排节点通过 runtime CRUD contract 执行，不直接调用插件或 SQL。
12. 删除字段和删除记录都写入审计；删除记录 V1 为物理删除。
13. 前端 Settings 中能从数据源进入数据模型列表和详情。
14. 未来 workspace HostExtension 能通过 scope provider 扩展多租户，不需要改写动态表结构。
...
`main_source` 动态表保留 `scope_id` 会让查询多一个过滤条件。V1 必须给动态表建立合适索引，例如 `(scope_id, created_at)` 和 owner 场景需要的 `(scope_id, created_by)`。单机开源版固定 `DEFAULT_SCOPE_ID`，该成本可控，且避免未来多租户插件反向改造所有动态表。
...
7. 编排通用数据模型节点。
8. workspace HostExtension scope provider 预留验证。
9. QA 验收与回归。
