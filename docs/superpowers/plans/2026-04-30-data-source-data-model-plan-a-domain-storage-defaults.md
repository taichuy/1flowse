...
- Reviewer rationale: `main_source`/null code uniqueness remains global intentionally. Plan A treats `main_source` as the built-in data source, and the approved design requires system-governed codes there to avoid multiple scopes creating duplicate physical table names. The proposed `unique(scope_kind, scope_id, code)` replacement was not adopted.
- Migration hardening: `20260430100000_data_model_status_exposure_defaults.sql` now scopes all `pg_constraint` existence checks with `conrelid` for the target table, so same-named constraints in other tables or schemas cannot cause required checks to be skipped.
- Reviewer gap fix: Data Model protection `owner_kind` now uses the design-source vocabulary `core`, `host_extension`, and `runtime_extension`; the domain mapping and PostgreSQL check constraint no longer define `data_source`.
- Code quality fix: `create_model` now resolves data source defaults through `actor.current_workspace_id`; PostgreSQL filters `data_source_instances` by `(id, workspace_id)`. Added service and repository coverage proving a foreign workspace `data_source_instance_id` is rejected while `main_source`/`None` defaults remain covered.
- Code quality fix: repository direct `create_model_definition` now rejects workspace-scoped Data Models whose explicit `data_source_instance_id` belongs to another workspace, while leaving System, `main_source`/`None`, and same-workspace data source behavior unchanged.
- Code quality fix: `update_model_status` now verifies model visibility in the actor workspace before writing, and the PostgreSQL status update also scopes the `update` predicate by workspace/system visibility. Added service and repository coverage proving a foreign workspace `model_id` is not mutated.
- Code quality fix: `create_scope_data_model_grant` and `update_scope_data_model_grant` now only target System Data Models and return controlled `model_definition` errors for workspace-scoped Data Models instead of leaking raw storage misses. Repository coverage now proves a System model can be granted to a workspace scope while workspace-scoped models are rejected through create/update repository paths without seeding invalid grant fixtures.
- Grant error-contract red/green evidence: after tightening repository assertions, `cargo test --manifest-path api/Cargo.toml -p storage-postgres model_definition_repository_tests` failed on the workspace-model grant create/update error-text assertions, then passed after the repository returned `ControlPlaneError::NotFound("model_definition")`.
- Code quality verification passed: `cargo fmt --manifest-path api/Cargo.toml --all`, `cargo test --manifest-path api/Cargo.toml -p domain modeling`, `cargo test --manifest-path api/Cargo.toml -p control-plane model_definition_service_tests`, `cargo test --manifest-path api/Cargo.toml -p storage-postgres model_definition_repository_tests`, and `cargo test --manifest-path api/Cargo.toml -p storage-postgres data_source_repository_tests`.
