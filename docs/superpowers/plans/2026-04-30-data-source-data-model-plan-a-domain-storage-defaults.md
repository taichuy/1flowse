...
- Code quality fix: `create_scope_data_model_grant` and `update_scope_data_model_grant` now only target System Data Models. Added repository coverage proving a System model can be granted to a workspace scope while workspace-scoped models are rejected for grant create and legacy grant update paths.
