...
- `sensitivity_level` 驱动的统一敏感访问控制、人工审核与通知闭环已确定为架构初期事项；在 `SensitiveResourceRecord / SensitiveAccessRequestRecord / ApprovalTicketRecord / NotificationDispatchRecord` 事实层与 `/api/sensitive-access/*` API 基础上，当前已经把同一套控制接到 runtime `credential resolve`、`mcp_query / authorized_context` 的 context read，以及 `ToolGateway` 的 tool invoke 主链：当 node/agent 解析 `credential://...`、读取命中 `workflow_context` 资源的上游 artifact，或调用命中 `local_capability` 资源的工具时，运行态现在都会创建/复用访问请求、在审批未完成时进入 `waiting_tool`，并在审批 `approved / rejected` 后通过既有 `RunResumeScheduler` 回到统一 `waiting/resume` 主链。对 `L2` context read，当前还会返回结构化 masked payload，而不是继续把原始内容直接透给下游；对 `local_capability` 的 `allow_masked`，当前默认等同“通过受控工具调用能力”而不是直出原始高敏内容。当前仍缺 publish export 的真实拦截挂点，以及更完整的通知 worker / inbox 落地；`allow_masked` 在 credential path 上也还没有真正区分“masked vs 明文”。
- 2026-03-15 最新复核结果：后端 `api/.venv/Scripts/uv.exe run pytest -q` 继续通过（`237 passed`）；除敏感访问控制的 runtime credential gating 与 `mcp_query / authorized_context` context read gating 外，本轮还补齐了 `ToolGateway` 的 sensitive tool invoke gating，并修正了 `llm_agent` waiting/resume checkpoint 在 JSON 字段上的 dirty-tracking 丢失问题，避免 direct tool node、llm_agent tool call 与 context read 的 waiting 语义继续分叉。changed-files `ruff check`、`pytest -q tests/test_runtime_sensitive_tool_gateway.py`（`2 passed`）和相邻 runtime 回归 `pytest -q tests/test_runtime_credential_integration.py tests/test_runtime_service_agent_runtime.py`（`20 passed`）已通过。后端全量 `ruff check` 的历史风格/整理债务仍未在本轮整体清零，因此稳定性基线继续提升，但还未达到“全仓库零告警”。
...
- `api/app/services/sensitive_access_control.py`：约 501 行，目前同时覆盖 resource/access request/ticket/notification 查询、默认策略、tool/context/credential matcher 与审批恢复，已经接近新的治理热点；若继续补 notification worker、更多 source matcher 或 policy plug-in，建议优先拆 query/mutation/policy helper，避免 service 主文件继续线性膨胀。
...
2. **P0：继续扩统一敏感访问控制闭环**
   - `SensitiveAccessRequest`、`ApprovalTicket`、`NotificationDispatch`、`/api/sensitive-access/*`、runtime `credential resolve`、`mcp_query / authorized_context` 的 context read 拦截、`ToolGateway` 的 tool invoke gating，以及审批后的 `waiting/resume` 调度都已落地；下一步优先把同一套控制继续挂到 publish export 入口，并补真实通知 worker / inbox，同时把 credential path 的 `allow_masked` 从“事实等同 allow”收成真正的 masked/handle 语义，避免统一治理仍停留在 credential/context/tool 三条局部主链。
