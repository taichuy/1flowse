...
- `HEAD` 提交 `153a20c feat: add runtime execution policy trace surface` 是对上一提交 `65950d3 docs: align graded execution architecture guidance` 的直接承接：`runtimePolicy.execution` 已落到 schema、编辑器和 run trace，但真实 Execution Adapter Registry、Tool Gateway 执行类映射和 `sandbox_code` 正式执行链仍未落地。
...
- 2026-03-15 复核结果：后端 `api/.venv/Scripts/uv.exe run pytest` 通过（219 passed），前端 `pnpm lint` 与 `pnpm exec tsc --noEmit` 通过；但后端全量 `ruff check` 仍有 74 条历史风格/整理债务，稳定性基线已显著提升但还未达到“全仓库零告警”。
...
- Runtime execution 目前仍经由 `llm_agent -> AgentRuntime`、`tool -> ToolGateway` 和 worker-inline 默认路径推进；`runtimePolicy.execution` 已能进入 workflow schema、node input 和 execution view，但真实 adapter dispatch、`sandbox_code` 和统一 Execution Adapter Registry 仍待落地。
...
1. **P0：把 graded execution 变成真实执行链**
   - 优先落地 Execution Adapter Registry、真实 adapter dispatch、`sandbox_code` 执行链，以及 tool/plugin 默认 execution class 映射，避免 `runtimePolicy.execution` 长期停留在“配置 + trace 可见”阶段。
