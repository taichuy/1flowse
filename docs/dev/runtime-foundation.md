...
- Graded execution is now a real code path concern instead of a docs-only decision: `runtimePolicy.execution` is available in schema validation, editor forms, and run traces, but actual node execution still defaults to worker-inline until the execution adapter registry lands.
...
- Runtime execution still flows through `llm_agent -> AgentRuntime`, `tool -> ToolGateway`, and worker-inline execution for the remaining nodes. As of 2026-03-15, `runtimePolicy.execution` is visible in workflow schema validation, node input `execution`, and execution view summaries, but real adapter dispatch, `sandbox_code`, and a unified execution adapter registry are still pending.
...
- The `workflow editor` inspector now exposes `runtimePolicy.execution / retry / join` as structured sections. The execution section resolves the default execution class first and only persists JSON when the node deviates from that default, while join candidates still collapse to the actual incoming edges on the canvas.
...
- Earlier placeholder ideas such as local `sandboxEnabled` switches are now being folded into the unified `runtimePolicy.execution.class / profile / timeoutMs / networkPolicy / filesystemPolicy` surface. The next step is to connect those settings to the real execution adapter registry instead of adding more scattered booleans.
...
- `web/components/workflow-node-config-form/runtime-policy-form.tsx`: 334 lines. The execution section has been split into `runtime-policy-execution-section.tsx` (159 lines) and `runtime-policy-helpers.ts` (152 lines); the parent form still orchestrates retry / join sections and should keep shrinking if more runtime controls are added.
...
1. **P0: turn graded execution config into a real execution chain**
   - `NodeType` vs `execution class` modeling, editor forms, and baseline trace fields are now in place. Next, prioritize the Execution Adapter Registry, tool/plugin default execution-level mapping, and real adapter dispatch so that `runtimePolicy.execution` no longer stops at config + trace only.
...
3. **P0: keep improving workflow editor completeness**
   - `runtimePolicy.execution / retry / join`, node `input/output schema`, and `llm_agent.toolPolicy` are already structured sections. The next editor step is workflow `publish` config, sensitive-access policy placement, and a clearer boundary between advanced JSON and structured forms.
