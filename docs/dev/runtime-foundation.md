...
3. **P0：继续收口 `WAITING_CALLBACK` 的 durable resume 语义**
   - callback ticket cleanup 与手动 cleanup 已能把“过期 ticket -> scheduled resume”一致接回主链，且 resume 调度已改成 after-commit 派发；2026-03-15 已补第一版 repeated expiry backoff、late callback 事件/摘要、execution view 的 callback lifecycle 展示，以及 published waiting surface 的同源摘要复用。下一步优先补最大重试/终止策略、callback source 聚合与 published callback drilldown 的进一步统一，避免外部 callback 型节点仍在边界场景里长期反复等待但缺少明确停机语义。
