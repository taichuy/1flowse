...
3. **P0：继续收口 `WAITING_CALLBACK` 的 durable resume 语义**
   - callback ticket cleanup 已能在后台把“过期 ticket -> immediate resume”接回主链；下一步优先补 operator 手动 cleanup 的一致行为、late callback / repeated waiting 的退避与摘要，以及更清晰的 run diagnostics 呈现，避免外部 callback 型节点仍在边界场景里反复等待或缺少可见性。
