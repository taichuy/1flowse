...
3. **P0：继续收口 `WAITING_CALLBACK` 的 durable resume 语义**
   - callback ticket cleanup、手动 cleanup 与“直接收到已过期 callback”三条入口，现都能把“过期 ticket -> scheduled resume”一致接回主链；重复 expiry 达到 `callback_ticket_max_expired_cycles` 时，也会统一写入 terminated 摘要并把 run / node run 明确终止失败。下一步优先补 callback source 聚合、published callback drilldown、通知 worker / inbox 与更细的 operator 控制面，避免 callback 型节点虽然已有停机边界，但排障与治理入口仍然分散。
