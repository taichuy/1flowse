...
2. **P0：继续扩统一敏感访问控制闭环**
   - `SensitiveAccessRequest`、`ApprovalTicket`、`NotificationDispatch`、`/api/sensitive-access/*`、runtime `credential resolve`、`mcp_query / authorized_context` 的 context read 拦截、`ToolGateway` 的 tool invoke gating、Run API `trace export` 的敏感导出控制、published endpoint invocation detail / cache inventory / invocation export 的人工详情查看控制，以及 publish 面板对 access-blocked 状态的最小 UI 落点都已落地；credential path 的 `allow_masked` 也已补成真正的 masked/handle 语义。下一步优先补真实通知 worker / inbox、approval timeline 与更细的 operator 控制面，避免统一治理仍停留在 credential/context/tool/export/details 几条局部主链。
