...
- `sensitivity_level` 驱动的统一敏感访问控制、人工审核与通知闭环已确定为架构初期事项；在 `SensitiveResourceRecord / SensitiveAccessRequestRecord / ApprovalTicketRecord / NotificationDispatchRecord` 事实层与 `/api/sensitive-access/*` API 基础上，当前已经把同一套控制接到 runtime `credential resolve`、`mcp_query / authorized_context` 的 context read、`ToolGateway` 的 tool invoke、Run API 的 `trace export`，以及 published endpoint invocation detail 的人工详情入口：当 node/agent 解析 `credential://...`、读取命中 `workflow_context` 资源的上游 artifact、调用命中 `local_capability` 资源的工具，尝试导出一个已经触达敏感资源的 run trace，或查看关联高敏 run 的 published invocation 详情时，运行态现在都会创建/复用访问请求，并在命中审批时复用同一套 `ApprovalTicket` / `NotificationDispatch` 事实层。对 `L2` context read，当前还会返回结构化 masked payload，而不是继续把原始内容直接透给下游；对 `local_capability` 的 `allow_masked`，当前默认等同“通过受控工具调用能力”而不是直出原始高敏内容。当前仍缺 published cache inventory / publish export 的真实拦截挂点，以及更完整的通知 worker / inbox 落地；`allow_masked` 在 credential path 上也还没有真正区分“masked vs 明文”。
- 2026-03-15 最新复核结果：后端 `api/.venv/Scripts/uv.exe run pytest -q` 继续通过（`241 passed`）；除敏感访问控制的 runtime credential gating、`mcp_query / authorized_context` context read gating、`ToolGateway` sensitive tool invoke gating 与 Run API `trace export` 之外，本轮继续把 published endpoint invocation detail 接到同一套 access request / approval 主链，并新增独立 `published_invocation_detail_access.py` 与 `run_sensitive_access_summary.py` 把“基于 run 已触达敏感资源，治理人类详情查看面”的判断从 route 中抽出，避免 published 详情治理继续散落在 activity route 与未来 export 入口里。changed-files `ruff check`、`pytest -q tests/test_published_invocation_detail_access.py tests/test_run_trace_export_access.py`（`4 passed`）与 `pytest -q tests/test_workflow_publish_routes.py -k 'published_invocation_detail or cache_entries or published_endpoints'`（`2 passed`）已通过；后端全量回归同样通过。后端全量 `ruff check` 的历史风格/整理债务仍未在本轮整体清零，因此稳定性基线继续提升，但还未达到“全仓库零告警”。
...
2. **P0：继续扩统一敏感访问控制闭环**
   - `SensitiveAccessRequest`、`ApprovalTicket`、`NotificationDispatch`、`/api/sensitive-access/*`、runtime `credential resolve`、`mcp_query / authorized_context` 的 context read 拦截、`ToolGateway` 的 tool invoke gating、Run API `trace export` 的敏感导出控制、published endpoint invocation detail 的人工详情查看控制，以及审批后的 `waiting/resume` 调度都已落地；下一步优先把同一套控制继续挂到 published cache inventory / publish export 入口，并补真实通知 worker / inbox，同时把 credential path 的 `allow_masked` 从“事实等同 allow”收成真正的 masked/handle 语义，避免统一治理仍停留在 credential/context/tool/export/details 几条局部主链。
