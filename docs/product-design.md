...
#### RuntimeFactLayerContract

```ts
type RuntimeFactLayerContract = {
  primaryFacts: {
    runs: 'workflow-level lifecycle and final outcome source of truth'
    nodeRuns: 'node-level execution and phase source of truth'
    runEvents: 'append-only timeline and streaming fact log'
    runArtifacts: 'raw payload, file and large result fact store'
  }
  derivedProjections: {
    runSnapshot: 'compact cross-entry status projection'
    executionView: 'operator and diagnostics projection'
    evidenceView: 'human/ai-friendly distilled evidence projection'
    publishedInvocationDetail: 'published surface projection'
    authorizedContext: 'mcp/runtime authorized read projection'
  }
}
```
...
- `runs` 负责工作流级事实，例如当前状态、当前节点、入口来源、绑定版本、恢复位置和最终结果摘要。
- `node_runs` 负责节点级事实，例如节点状态、phase、waiting reason、execution class、重试与节点输出摘要。
- `run_events` 负责追加式事件时间线、流式输出和关键状态变迁；它是观察与回放事实，不承载大体量原始载荷。
- `run_artifacts` 负责原始大结果、文件、prompt/response 快照、tool raw output 和可追溯引用；它不是主状态总线。
- `tool_call_records`、`ai_call_records` 等侧边事实表必须继续通过 `run_id / node_run_id` 挂回同一事实层，而不是自成第二套执行真相。
- `run_snapshot`、`execution_view`、`evidence_view`、`published invocation detail`、`authorized_context` 都是基于同一事实层派生的读模型或投影视图；任何投影都不能反过来成为新的事实源。
...
- 投影不是真相：给 MCP、发布接口、调试面板、发布治理页或运营入口使用的 `run_snapshot`、execution view、evidence view 只能从事实层派生，不能各自维护旁路状态。
...
- 调试面板、发布 detail、operator 入口和 AI 自治读模型应优先消费同一批运行事实投影，例如 `run_snapshot`、execution view、evidence view，而不是每个入口自行拼装状态。
