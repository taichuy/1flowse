...
summary: `tenant-runtime` 与 `runner/plugin` 不允许注册或扩展系统接口，只能实现宿主预定义的白名单能力槽位；接口扩展能力仅开放给 `core` 与 `host-extension`。
keywords:
  - plugin
  - host-extension
  - tenant-runtime
...
- 如果 `tenant-runtime` 或 `runner/plugin` 能直接注册系统接口，未来多租户场景下很容易绕过宿主级鉴权、租户隔离和敏感数据边界。
...
- `tenant-runtime` 与 `runner/plugin` 不允许注册接口，也不允许扩展系统接口。
- `tenant-runtime` 与 `runner/plugin` 只能实现宿主预定义的白名单能力槽位，不能自行发明新的扩展点。
- `tenant-runtime` 与 `runner/plugin` 只能按权限读取或操作宿主开放的数据能力，不能直接触达系统控制面接口、底层存储和 HTTP 契约。
- 当前白名单能力口径为：
  - `tenant-runtime`：`runtime.query.scope_resolver`、`runtime.record.validator`、`runtime.field.default_value`、`runtime.field.computed_value`
  - `runner/plugin`：`runner.node.execute`、`runner.datasource.read`、`runner.datasource.schema`、`runner.publish.render`、`runner.publish.deliver`
