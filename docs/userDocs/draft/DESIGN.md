...
**日期：2026-04-12**
**状态：权威执行版（单一事实源）**
**变更说明：重建为双层结构 — 视觉执行层 + 工作区规则层**

> Codex 可直接按本文件编码，不需要查阅其他 planner 文档。Part 1 给出 token 与组件 recipe；Part 2 给出工作区边界与交互规则。两部分同等权威。
...
1Flowse 前端采用**工具型控制台风格**：高对比、低装饰、状态语义清晰。可直接对照的参照系是 Linear、Vercel dashboard、Ant Design Pro（已定制 token 版）——不是营销页，不是消费级 App。
