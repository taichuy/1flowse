...
- `docs/dev/`：当前共享协作索引目录，当前主要维护 `README.md` 与 `team-conventions.md`
- `docs/adr/`：架构与长期协作决策记录
- `docs/history/`：不再作为共享 history 使用，当前只保留迁移说明
- `docs/expired/`：废弃但保留历史价值的文档
- `.agents/skills/`：AI 协作开发时优先复用的元流程技能、专项流程、审查规则和参考资料，例如持续自治开发、开发收尾、审查、测试和组件拆分
...
- 产品/技术基线、开源/商业策略、共享协作约定和本地私有留痕应继续分层维护，避免把目标设计、团队规则和个人过程记录混写成同一事实源。
...
- AI 协作开发默认先读 `AGENTS.md`、产品/技术/策略基线、`docs/dev/` 共享协作约定，再判断是否需要“元流程技能 + 领域技能”组合；如当前本地开发者已维护 `docs/.private/` 私有连续性文档，再按需补读。
